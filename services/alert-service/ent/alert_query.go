...
func (aq *AlertQuery) QueryRule() *AlertRuleQuery {
	query := (&AlertRuleClient{config: aq.config}).Query()
...
func (aq *AlertQuery) WithRule(opts ...func(*AlertRuleQuery)) *AlertQuery {
	query := (&AlertRuleClient{config: aq.config}).Query()
...
func (aq *AlertQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Alert, error) {
	...
	if query := aq.withRule; query != nil {
		if err := aq.loadRule(ctx, query, nodes, nil,
...
func (aq *AlertQuery) loadRule(ctx context.Context, query *AlertRuleQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *AlertRule)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Alert)
	for i := range nodes {
		if nodes[i].alert_rule_alerts == nil {
			continue
		}
		fk := *nodes[i].alert_rule_alerts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "alert_rule_alerts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
